...
namespace DashyBoard.Infrastructure.External
{
    public sealed class WeatherApiClient : IWeatherApiClient
    {
        ...
        public async Task<RawWeatherForecastDto> GetWeatherForecastAsync(string longi, string lati, CancellationToken ct)
        {
            var result = await _http.GetFromJsonAsync<RawWeatherForecastDto>($"forecast?latitude={lati}&longitude={longi}&hourly=temperature_2m,weather_code,wind_speed_10m,precipitation,precipitation_probability&timezone=auto", ct);
            return result ?? throw new InvalidOperationException("Empty response from Weather API.");
        }
    }
}
