...
namespace DashyBoard.Infrastructure.External
{
    public sealed class WeatherApiClient : IWeatherApiClient
    {
        ...
        {
            var result = await _http.GetFromJsonAsync<RawCurrentWeatherDto>($"forecast?latitude={lati}&longitude={longi}&current=temperature_2m,weather_code,precipitation,wind_speed_10m,precipitation_probability,apparent_temperature,is_day", ct);
